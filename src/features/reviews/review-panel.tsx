...
export function ReviewPanel({ projectId, canPublish }: { projectId: string; canPublish: boolean }) {
  ...
  useEffect(() => { if (!active?.canPlay) return; void (async () => { const response = await fetch(`/api/projects/${projectId}/reviews/${active.id}/playback`); if (response.ok) { const result = await response.json(); setPlaybackUrl(result.data.url); } })(); }, [active?.id, active?.canPlay, projectId]);
