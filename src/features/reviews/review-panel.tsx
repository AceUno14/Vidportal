...
export function ReviewPanel({ projectId, canPublish, canDecide, onDecision }: { projectId: string; canPublish: boolean; canDecide: boolean; onDecision?: () => void }) {
  ...
  useEffect(() => {
    ...
    // The selected review intentionally clears a stale signed URL before requesting a new one.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setPlaybackUrl(null);
    setPlaybackError(null);
    void (async () => { const response = await fetch(`/api/projects/${projectId}/reviews/${active.id}/playback`); if (response.ok) { const result = await response.json(); setPlaybackUrl(result.data.url); return; } const result = await response.json().catch(() => null); setPlaybackError(result?.error ?? "Private playback is unavailable for this review version."); })();
  ...
  async function decide(decision: "APPROVED" | "CHANGES_REQUESTED") { if (!active) return; setSaving(true); const response = await fetch(`/api/projects/${projectId}/reviews/${active.id}/decision`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ decision }) }); setSaving(false); if (response.ok) { await load(); onDecision?.(); } }
